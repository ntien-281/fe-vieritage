...
const Map = () => {
  ...
  const markers = [
    {
      title: 'Đêm hội xuân của FLC',
      latitude: 10.9432,
      longitude: 106.83185,
    },
    {
      title: 'Đêm nhạc TP Bank',
      latitude: 10.876541,
      longitude: 106.721825,
    },
    {
      title: 'Hội thi điêu khắc đã mỹ nghệ Non Nước',
      latitude: 10.986549,
      longitude: 106.75181,
    },
  ]
    ...
    <View style={styles.container}>
      ...
      <MapView style={styles.map} region={mapRegion} provider='google'>
        {markers
          .filter(
            (marker) =>
              getDistanceFromLatLonInKm(mapRegion.latitude, mapRegion.longitude, marker.latitude, marker.longitude) < 10
          )
          .map((marker) => (
            <Marker
              coordinate={{
